...
export function buildDashboardSql(
  type: 'line' | 'bar' | 'pie' | 'scatter' | 'preview',
  cols: { dateCol?: string; catCol?: string; numCol?: string; secondNumCol?: string },
  agg: 'sum' | 'avg' | 'count' = 'sum'
): string {
  const q = quoteSqlIdentifier;
  const table = 'dataset';
  const aggFn = (col: string) => {
    if (agg === 'count') return 'COUNT(*)';
    if (agg === 'avg') return `AVG(${q(col)})`;
    return `SUM(${q(col)})`;
  };

  switch (type) {
    case 'line':
      if (cols.numCol && cols.dateCol) {
        return `SELECT ${q(cols.dateCol)}, ${aggFn(cols.numCol)} AS value FROM ${table} GROUP BY 1 ORDER BY 1`;
      }
      return `SELECT ${q(cols.dateCol || cols.catCol || 'rowid')}, COUNT(*) AS value FROM ${table} GROUP BY 1 ORDER BY 1`;

    case 'bar':
      if (cols.numCol && cols.catCol) {
        return `SELECT ${q(cols.catCol)}, ${aggFn(cols.numCol)} AS value FROM ${table} GROUP BY 1 ORDER BY 2 DESC LIMIT 20`;
      }
      return `SELECT ${q(cols.catCol || 'rowid')}, COUNT(*) AS value FROM ${table} GROUP BY 1 ORDER BY 2 DESC LIMIT 20`;

    case 'pie':
      if (cols.numCol && cols.catCol) {
        return `SELECT ${q(cols.catCol)}, AVG(${q(cols.numCol)}) AS value FROM ${table} GROUP BY 1 ORDER BY 2 DESC LIMIT 8`;
      }
      return `SELECT ${q(cols.catCol || 'rowid')}, COUNT(*) AS value FROM ${table} GROUP BY 1 ORDER BY 2 DESC LIMIT 8`;

    case 'scatter':
      if (cols.numCol && cols.secondNumCol) {
        return `SELECT ${q(cols.numCol)} AS x, ${q(cols.secondNumCol)} AS y, ${cols.catCol ? q(cols.catCol) : 'NULL'} AS label FROM ${table} WHERE ${q(cols.numCol)} IS NOT NULL AND ${q(cols.secondNumCol)} IS NOT NULL LIMIT 500`;
      }
